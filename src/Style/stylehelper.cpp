...
QColor StyleHelper::m_baseColor(0x880000);
...
void StyleHelper::menuGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect)
{
    QString key;
    key.sprintf("mh_toolbar %d %d %d %d %d", spanRect.width(), spanRect.height(), clipRect.width(),
                                             clipRect.height(),QColor(0x666666).rgb() /*StyleHelper::baseColor().rgb()*/);;
    QPixmap pixmap;
    QPainter *p = painter;
    QRect rect = clipRect;
    if (StyleHelper::usePixmapCache() && !QPixmapCache::find(key, pixmap)) {
    ...
    QColor menuColor = mergedColors(QColor(0x666666), QColor(244, 244, 244), 25);
